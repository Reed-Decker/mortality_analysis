...
Due to limitations on how many results the CDC Wonder page is able to give, I had to submit separate queries for each race category. As such, my first step is to merge this back together into a single dataset. Additionally, none of the original datasets contain a race column, so I add one into each dataset before merger.
...
It looks like we should be able to convert Crude.Rate to float now. We can also tell that the Population column must be character, rather than integer, because it has several "Not Applicable" text values. With the crude rate I filled those with 0s, because we already had set up a way to filter out values we don't want, and it didn't risk breaking any packages by feeding them NA data. For population, however, we don't have as easy a way to filter out formerly NA values. I don't want to count NA populations as 0 either, because I think it might sway any averages on population I might want to do later. Instead, I think I'd like to just convert the "Not Applicable" strings into NA cells recognized by R. The easiest way to do this is to just convert the population column to integer. That should automatically turn all the character strings to NA. If this winds up being incompatible with some of our functions later on, we can work out a new solution, but for now this is faster and should allow us to avoid adding any additional columns. 

Before we convert those two columns, let's see what the structure of the rest of the dataframe is like.

```{r check_structure}
str(dat)
```

It looks like our various code columns are already set to be factors, which is good, but Crude.Rate.Reliability and Race will also need to be set as factors. Year and Deaths make sense as integers to me, so we can leave those as they are, and we already know that Crude.Rate should be float, and Population should definitely be integer rather than factor. 

```{r set_data_type}
dat$Crude.Rate.Reliability <- as.factor(dat$Crude.Rate.Reliability)
dat$Race <- as.factor(dat$Race)

dat$Crude.Rate <- as.float(dat$Crude.Rate)
